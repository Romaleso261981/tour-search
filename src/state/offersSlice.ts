...
export const searchOffersByCountry = createAsyncThunk<
  OfferItem[],
  { countryId: string },
...
>(
  ...
  async ({ countryId }, { getState, dispatch, rejectWithValue, requestId }) => {
    try {
      ...
      if (prevToken) {
        try {
          await stopActiveSearch(prevToken);
        } catch {}
      }
      ...
      const mapped: OfferItem[] = Object.values(prices)
        .map((p: any) => {
          const hotel = (Object.values(hotels) as any[]).find(
            (h) => String((h as any).id) === String(p.hotelID)
          ) as any;
          return {
            id: p.id,
...
const offersSlice = createSlice({
  ...
  reducers: {
    ...
    setHotelsCache(
      state,
      action: PayloadAction<{ countryId: string; hotels: any }>
    ) {
...
export const { setOffers, clearOffers, setActiveToken, setHotelsCache } =
  offersSlice.actions;
