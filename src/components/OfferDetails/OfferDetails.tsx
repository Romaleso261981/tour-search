...
type Hotel = { id: number; name: string; img?: string; description?: string; services?: Record<string, string> };
type Price = { id: string; amount: number; currency: string; startDate: string; endDate: string };
      ...
      {!loading && !error && hotel && price && (
        <div className={styles.content}>
          <div className={styles.header}>
            {hotel.img && <img className={styles.image} src={hotel.img} alt={hotel.name} />}
            <div className={styles.info}>
              <h2 className={styles.title}>{hotel.name}</h2>
              <div className={styles.dates}>{price.startDate} — {price.endDate}</div>
              <div className={styles.price}>{price.amount} {price.currency.toUpperCase()}</div>
            </div>
          </div>
